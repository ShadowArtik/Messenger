...
картинок** (через REST `/api/files`, маркер `[IMG]{id}` в тексте сообщения), роли в группах
(owner/admin/member: добавить/кикнуть/передать владение/переименовать/выйти), бот-помощник.

## Не реализовано (на будущее)
- Удаление/редактирование сообщений (нужен id сообщения через весь конвейер — UUID на клиенте).
