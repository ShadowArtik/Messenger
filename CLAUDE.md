...
- `controller/ChatActionsHandler` — действия с чатом: создание приватного, очистка, переименование,
  удаление, отправка сообщения, typing, **отправка картинки** (`attachImage`),
  **редактирование/удаление сообщения** (`startEdit`/`commitEdit`/`deleteMessage`, режим правки в поле ввода).
- `controller/ProfileHandler` — профиль текущего юзера (карточка сбоку + «Edit profile»).
- `controller/WebSocketMessageHandler` — обработка входящих realtime-сообщений (новое сообщение, typing,
  онлайн, события групп, история, очистка, **прочтение `MESSAGES_READ`**,
  **удаление/редактирование `MESSAGE_DELETED`/`MESSAGE_EDITED`**).
- `view/Avatars` — построение круглых аватаров (база, онлайн-точка, бот-бейдж).
- `view/LastSeen` — форматирование «last seen …» (Today/Yesterday/дата).
- `view/ImageStore` — скачивание+кэш картинок-вложений по id.
- `view/cell/MessengerCells` — фабрики ячеек (`contactCell`, `messageCell`, участники группы);
  тут же рендер картинок в пузыре, ✓/✓✓ и разделители дней.
...
## Реализованные фичи
Приватные и групповые чаты, история, печатает…, онлайн + **last seen**, **галочки ✓/✓✓** (приватные и
группы, по правилу «прочитал хоть кто-то»), счётчики непрочитанных, **разделители дней**, **отправка
картинок** (через REST `/api/files`, маркер `[IMG]{id}` в тексте сообщения),
**редактирование и удаление сообщений** (правый клик по своему сообщению; `clientId`-UUID
протянут через модель → XML → БД, рассылка `MESSAGE_DELETED`/`MESSAGE_EDITED`), роли в группах
(owner/admin/member: добавить/кикнуть/передать владение/переименовать/выйти), бот-помощник.

## Не реализовано (на будущее)
- Авторизация на сервере (REST/WS открыты, любой может подключиться под любым userId).
- Картинки: только PNG/JPG/JPEG/GIF/BMP (JavaFX не умеет webp).
